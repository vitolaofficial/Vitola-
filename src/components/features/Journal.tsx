...
export function Journal({ entries, cigars, onAdd }: JournalProps) {
  ...
  const moodIcons = {
    great: { icon: Smile, label: t("journal.moodExcellent" as TranslationKey), color: "text-emerald-400" },
    good: { icon: Meh, label: t("journal.moodGood" as TranslationKey), color: "text-amber-400" },
    neutral: { icon: Frown, label: t("journal.moodMeh" as TranslationKey), color: "text-muted-foreground" },
  };
      ...
      <div className="relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs font-ui tracking-[0.4em] uppercase text-gold/50 mb-2">{t("journal.tastingNotes" as TranslationKey)}</p>
              <h1 className="font-display text-4xl md:text-5xl text-cream font-bold mb-3 text-gold-gradient">{t("journal.title" as TranslationKey)}</h1>
              <p className="font-serif-body text-cream/35">
                {entries.length === 0 ? t("journal.emptyDesc" as TranslationKey) : t("journal.count" as TranslationKey, { count: entries.length, s: entries.length === 1 ? "" : "s" })}
              </p>
            </div>
            <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-6 py-3 bg-gold text-[#0a0604] font-ui font-bold text-xs tracking-[0.2em] uppercase hover:shadow-[0_0_40px_rgba(197,160,89,0.2)] transition-all duration-500">
              <Plus size={14} />
              {t("journal.newEntry" as TranslationKey)}
            </button>
          </div>
        ...
        {entries.length === 0 && !showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
            ...
            <p className="font-display text-2xl text-cream mb-3">{t("journal.emptyTitle" as TranslationKey)}</p>
            <p className="font-serif-body text-muted-foreground mb-8 max-w-sm mx-auto">{t("journal.emptyText" as TranslationKey)}</p>
            <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 px-8 py-3 border border-gold text-gold font-ui text-sm tracking-wider uppercase hover:bg-gold hover:text-mahogany transition-all duration-300">
              <Plus size={14} />
              {t("journal.logFirst" as TranslationKey)}
            </button>
          </motion.div>
        )}
            ...
            {entries.map((entry, i) => {
              ...
              return (
                <motion.div key={entry.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                  className="gradient-card border border-border hover:border-gold/30 p-5 card-premium">
                  ...
                  <div className="flex gap-4 mb-3 text-xs font-ui">
                    <span className="text-muted-foreground">{t("journal.burn" as TranslationKey)} <span className="text-foreground ml-1">{entry.burnScore}/5</span></span>
                    <span className="text-muted-foreground">{t("journal.draw" as TranslationKey)} <span className="text-foreground ml-1">{entry.drawScore}/5</span></span>
                  </div>
                  {entry.notes && (
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <div className="absolute inset-0 bg-mahogany/90 backdrop-blur-md" onClick={() => setShowForm(false)} />
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
                ...
                <div className="p-6">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="font-display text-2xl text-cream">{t("journal.logSession" as TranslationKey)}</h2>
                    <button onClick={() => setShowForm(false)} className="text-muted-foreground hover:text-cream" title="Close"><X size={18} /></button>
                  </div>
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">{t("journal.cigarName" as TranslationKey)} *</label>
                      ...
                      <div>
                        <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">{t("journal.brand" as TranslationKey)}</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">{t("journal.location" as TranslationKey)}</label>
                        <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="e.g. Terrace, lounge"
                          className="w-full bg-input border border-border px-3 py-2.5 text-sm text-foreground font-ui placeholder:text-muted-foreground/50 focus:outline-none focus:border-gold/50 transition-colors" />
                      </div>
                      <div>
                        <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">{t("journal.pairedWith" as TranslationKey)}</label>
                        <input value={form.pairing} onChange={(e) => setForm({ ...form, pairing: e.target.value })} placeholder="e.g. Scotch, espresso"
                          className="w-full bg-input border border-border px-3 py-2.5 text-sm text-foreground font-ui placeholder:text-muted-foreground/50 focus:outline-none focus:border-gold/50 transition-colors" />
                      </div>
                    </div>
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-2 block">{t("journal.mood" as TranslationKey)}</label>
                    ...
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-2 block">{t("journal.flavorsDetected" as TranslationKey)}</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-2 block">{t("journal.burnQuality" as TranslationKey)} ({form.burnScore}/5)</label>
                        <input type="range" min={1} max={5} value={form.burnScore} onChange={(e) => setForm({ ...form, burnScore: parseInt(e.target.value) })} className="w-full accent-gold" title="Burn Score" />
                      </div>
                      <div>
                        <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-2 block">{t("journal.drawQuality" as TranslationKey)} ({form.drawScore}/5)</label>
                        <input type="range" min={1} max={5} value={form.drawScore} onChange={(e) => setForm({ ...form, drawScore: parseInt(e.target.value) })} className="w-full accent-gold" title="Draw Score" />
                      </div>
                    </div>
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-2 block">{t("journal.overallRating" as TranslationKey)}</label>
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <button key={s} type="button" onClick={() => setForm({ ...form, rating: s * 20 })} className="transition-transform hover:scale-110" title={`Rate ${s} stars`}>
                    ...
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">{t("journal.notes" as TranslationKey)}</label>
                    ...
                    <div className="flex gap-3 pt-2">
                      <button type="button" onClick={() => setShowForm(false)}
                        ...
                        {t("journal.cancel" as TranslationKey)}
                      </button>
                      <button type="submit"
                        ...
                        {t("journal.save" as TranslationKey)}
                      </button>
                    </div>
                  </form>
                </div>
